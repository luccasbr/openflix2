...
/**
 * RTCPeerConnection + sinalização por WebSocket (WSS).
 * Env:
...
function pcFactory(iceServers, signalUrl, role, room, token) {
  ...
  // cliente inicia renegociação quando necessário
  ...
  async function ensureOffer() {
    ...
    // se já temos remote + SCTP, nada a fazer
    if (pc.remoteDescription && pc.sctp) return;
