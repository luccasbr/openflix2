...
/**
 * Fábrica de RTCPeerConnection + sinalização por WebSocket (WSS).
 * Env:
...
function pcFactory(iceServers, signalUrl, role, room, token) {
  ...
  function wireWS() {
    ...
    ws.on('message', async (raw) => {
      ...
      if (m.type === 'peer-ready' && role === 'client') {
        // cliente inicia a primeira oferta, se necessário
        ensureOffer();
      }
      if (m.type === 'signal') {
        try {
  ...
  // Cria offer apenas quando NECESSÁRIO (primeira vez / reconexão)
  async function ensureOffer() {
    // evita oferta repetida: só se ainda não tem remoteDescription
